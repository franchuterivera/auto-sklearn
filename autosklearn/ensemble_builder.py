...
class EnsembleBuilder(object):
        ...
        if not sorted_keys:
            ...
            if num_keys > num_dummy:
                self.logger.warning("No models better than random - using Dummy Score!"
                                    ...
                                    num_dummy)
            sorted_keys = [
                (k, v["ens_score"], v["num_run"], v['instance'], v['level']) for k, v in self.read_scores.items()
    ...
    def _get_list_of_sorted_preds(self):
        ...
        sorted_keys = list(reversed(sorted(
            [
                # NOTICE that we add after numrun because code relies on numrun in position 2
