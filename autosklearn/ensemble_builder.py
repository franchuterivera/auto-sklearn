...
def ensemble_builder_process(
    ...
    event = dask.distributed.Event(event)
    ...
    while True:
        ...
        if event.is_set():
            warnings.warn(
                "Terminate ensemble building because main process requested termination")
            break
        ...
        # COntext manager so that we delete the object so this process
        # doesn;t consume memory
        # https://stackoverflow.com/questions/36002705/python-with-not-deleting-object
        # with EnsembleBuilder(
        #
        # ) as ensemble:
    ...
    print(f"going to return {history}\n\n")
    return history
...
class EnsembleBuilder(object):
    ...
    def main(self, time_left, iteration):
        ...
        :return:
        """
        print(f"started main execution with backend={id(self.backend)}")
