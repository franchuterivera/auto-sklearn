...
class EnsembleBuilderManager(IncorporateRunResultCallback):
    ...
    -------
        ...
        if bbc_cv_strategy not in [
            ...
            'autosklearnBBCScoreEnsemblePERT',
        ...
        self.early_stop_oob = early_stop_oob
        print(f"self.ensemble_size={self.ensemble_size} and strategy={self.bbc_cv_strategy} with self.bbc_cv_sample_size={self.bbc_cv_sample_size} self.bbc_cv_n_bootstrap={self.bbc_cv_n_bootstrap} seed={self.seed}")
        ...
        if len(self.futures) == 0:
            ...
            try:
                ...
                self.futures.append(dask_client.submit(
                    ...
                    early_stop_oob=self.early_stop_oob,
                ))
...
class EnsembleBuilder(object):
    def __init__(
        ...
        self.early_stop_oob = early_stop_oob
        self.current_bbc_boot = None
        # The following hold the bootstrap generated by the AutoML
        ...
        self.logger.debug(f"early_stop_oob={early_stop_oob}")
        # ---------------------------------------------------------------------
    ...
    def run(
        ...
        while True:
            ...
            print(f"ERROR: change the time limit back")
            ...
            if safe_ensemble_script.exit_status in [pynisher.MemorylimitException, pynisher.TimeoutException]:
    ...
    def main(self, time_left, iteration, return_predictions):
        ...
        if ensemble is not None:
            if self.bbc_cv_strategy == 'autosklearnBBCEnsembleSelectionFULL':
                # Reset the damage!
                self.current_bbc_boot = None
            ...
            try:
                ...
                score = calculate_score(
                    ...
                    bootstrap_indices=None if self.bbc_cv_strategy is None or self.bbc_cv_strategy not in [
                        'autosklearnBBCScoreEnsemble',
                        'autosklearnBBCScoreEnsembleALLIB',
                        # Not in here in purpose as we do not want pre-filter correction
                        #'autosklearnBBCScoreEnsembleNOPREFILTER',
                        'autosklearnBBCScoreEnsembleAVGMDEV',
                        'autosklearnBBCScoreEnsemblePercentile',
                        'autosklearnBBCScoreEnsemblePERT',
		        ...
		        'autosklearnBBCEnsembleSelectionPreSelectInES',
                    ...
                    oob=False if self.bbc_cv_strategy in ['autosklearnBBCEnsembleSelectionPreSelectInES'
                                                          'autosklearnBBCScoreEnsembleALLIB',
                                                          'autosklearnBBCEnsembleSelectionPreSelectInESRegularizedEnd',
                                                          'autosklearnBBCEnsembleSelectionPreSelectInESRegularizedES',
                                                          ] else True,
    ...
    def bootstrap_indices_generator(self) -> List[List[int]]:
        """
        Interface to the bootstrap algorithm. If we have to do bootstrap bias correction,
        this procedure will return the bootstrap indices and leave the OOB calculation
        for the consumer. This is not time efficient, but memory efficient as having 2
        lists OOB and in boot indices is very memory consuming for big datasets
        ...
        # We only load the indices once
        # This is a bit of a lie, because if the ensemble builder
        # is re-started, we read this again (like on a pynisher kill)
        # But the intention is to read the npy file once!
        if self.prediction_indices_inb is None:
            ...
            if not os.path.exists(self.backend._get_bootstrap_inb_filename()):
                ...
                self.backend.save_bootstrap(self.y_true_ensemble, random_state=self.random_state)
    ...
    def fit_ensemble(self, selected_keys: list):
        ...
        ensemble = EnsembleSelection(
            ...
            # In case the strategy is 'autosklearnBBCEnsembleSelection',
            # we directly modify the data to train
            # so we should not provide the run indices
            # That is, look at the if below that involves
            # autosklearnBBCEnsembleSelection. We modify the data before
            # calling fit in this strategy
            bootstrap_indices=self.bootstrap_indices_generator(
            ) if self.bbc_cv_strategy in ['autosklearnBBCScoreEnsemble',
                                          'autosklearnBBCScoreEnsembleALLIB',
                                          'autosklearnBBCScoreEnsembleNOPREFILTER',
                                          'autosklearnBBCScoreEnsembleMAX',
                                          'autosklearnBBCScoreEnsembleMAXWinner',
        ...
        try:
            ...
            if self.bbc_cv_strategy == 'autosklearnBBCEnsembleSelectionFULL':
                weights = {}
                for b, indices in enumerate(self.bootstrap_indices_generator()):
                    weights[b]={}
                    self.current_bbc_boot = b
                    self.y_true_ensemble = self.backend.load_targets_ensemble()
                    self.y_true_ensemble = np.take(self.y_true_ensemble, indices, axis=0)
                    # Start from scratch
                    self.read_scores = {}
                    self.read_preds = {}
                    if not self.score_ensemble_preds():
                        raise NotImplementedError("I expect all predictions to be there")
                    candidate_models = self.get_n_best_preds()
                    if not candidate_models:  # no candidates yet
                        raise NotImplementedError("I expect all predictions to be there")
                    predictions_train = [self.read_preds[k][Y_ENSEMBLE] for k in candidate_models]
                    include_num_runs = [
                        (
                            self.read_scores[k]["seed"],
                            self.read_scores[k]["num_run"],
                    ...
                    ensemble.fit(predictions_train,
                                 self.y_true_ensemble,
                                 include_num_runs)
                    # We need to make sure for each identifier there is a weight
                    # maybe an identifier was not selected and that is not ok
                    for i, item in enumerate(include_num_runs):
                        if item in weights[b]:
                            raise NotImplementedError(f"We only expect one weight per identifier")
                        weights[b][item] = ensemble.weights_[i]

                # Autocomplete missing identifiers with 0 weights, meaning they were not used in a boot
                all_identifiers = set()
                for b, boot_weights in weights.items():
                    for k in boot_weights.keys():
                        all_identifiers.add(k)
                for b, boot_weights in weights.items():
                    for k in list(all_identifiers):
                        if k not in weights[b]:
                            weights[b][k] = 0
                all_identifiers = list(all_identifiers)
                all_weights_averaged = []
                for k in all_identifiers:
                    all_weights_averaged.append(
                        np.mean([weights[b][k] for b in weights.keys()])
                    )

                ensemble.identifiers_ = all_identifiers
                ensemble.weights_ = np.array(all_weights_averaged).astype(dtype=np.float32)
            ...
            elif self.bbc_cv_strategy is not None and 'autosklearnBBCEnsembleSelection' in self.bbc_cv_strategy:
                weights = []
                number_of_samples = self.y_true_ensemble.shape[0]
                for indices in self.bootstrap_indices_generator():
                    ensemble.fit(np.take(predictions_train, indices, axis=1),
                                 ...
                                 include_num_runs,
                                 early_stop_oob_bag=None if not self.early_stop_oob else np.setdiff1d(list(range(number_of_samples)), indices),
                                 )
                    weights.append(ensemble.weights_)

                print(f"Produced weights={weights}")
                ...
                # Reduce the model capacity if a lot of model exists
                if self.bbc_cv_strategy == 'autosklearnBBCEnsembleSelectionPreSelectInESRegularizedEnd' and np.count_nonzero(ensemble.weights_) > self.ensemble_size//2:
                    print(f"weights before = {ensemble.weights_}")
                    print(f"Reducing ensemble={ensemble.weights_ <= ensemble.weights_.max()/10}")
                    quarter = np.count_nonzero(ensemble.weights_)//4
                    # This is too aggressive -- ensemble.weights_[ensemble.weights_ <= ensemble.weights_.max()/10] = 0
                    # Just do up to a quarter
                    ensemble.weights_[ensemble.weights_ < np.sort(ensemble.weights_[ensemble.weights_ > 0])[quarter]] = 0

                # TODO: Like in every stage of ensemble selection do regularization 'autosklearnBBCEnsembleSelectionPreSelectInESRegularizedES',
    ...
    def _read_np_fn(self, path):
        ...
        with open_method(path, 'rb') as fp:
            ...
            if self.bbc_cv_strategy == 'autosklearnBBCEnsembleSelectionFULL' and self.current_bbc_boot is not None:
                indices = self.bootstrap_indices_generator()[self.current_bbc_boot]
                return np.take(predictions, indices, axis=0)
            return predictions
